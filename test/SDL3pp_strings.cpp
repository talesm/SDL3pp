...
{
  std::string result;
  auto test = [&](StringParam str) { result = str; };
...
TEST_CASE("SourceBytes")
{
  ...
  {
    const char bytes[] = "Test";
    SourceBytes sb{bytes, 0};
    CHECK(sb.data() == nullptr);
    CHECK(sb.size_bytes() == 0);
  }
  SUBCASE("nullptr pointer with non 0 size")
  {
    SourceBytes sb{(const char*)(nullptr), 5};
    CHECK(sb.data() == nullptr);
    CHECK(sb.size_bytes() == 0);
  }
...
TEST_CASE("TargetBytes")
{
  ...
  {
    char bytes[] = "Test";
    TargetBytes sb{bytes, 0};
    CHECK(sb.data() == nullptr);
    CHECK(sb.size_bytes() == 0);
  }
  SUBCASE("nullptr pointer with non 0 size")
  {
    TargetBytes sb{(char*)(nullptr), 5};
    CHECK(sb.data() == nullptr);
    CHECK(sb.size_bytes() == 0);
  }
  SUBCASE("Sint32 array")
