...
function transformApi(config) {
    ...
    for (const [sourceName, sourceFile] of Object.entries(source.files)) {
        ...
        utils_1.system.log(`Transforming api ${sourceName} => ${targetName}`);
...
class ApiContext {
    ...
    addGlossary(sourceName, entry) {
        this.glossary[sourceName] = (0, utils_1.deepClone)(entry);
    }
    /**
     *
     * @param {string} sourceName
    ...
    addName(originalType, targetType) {
        if (!this.nameMap[originalType])
            this.nameMap[originalType] = targetType;
    }
    /**
    ...
    isAfterMinVersion(version) {
        ...
        if (!tag)
            return false;
        if (version.major > tag.major)
            return true;
        if (version.major < tag.major)
            return false;
        if (version.minor > tag.minor)
            return true;
        if (version.minor < tag.minor)
            return false;
        if (version.patch > tag.patch)
            return true;
...
function expandTypes(sourceEntries, file, context) {
    ...
    for (const [sourceName, sourceEntry] of Object.entries(sourceEntries)) {
        if (context.blacklist.has(sourceName))
            continue;
        if (!isType(sourceEntry.kind))
            continue;
    ...
    for (const [targetName, targetDelta] of Object.entries(file.transform)) {
        if (targetDelta.enum)
            expandEnumeration(targetName, { kind: "alias", name: targetName }, targetName, targetDelta);
    }
    /**
     *
    ...
    function expandResource(sourceName, sourceEntry, targetName, targetEntry) {
        ...
        if (hasScoped)
            referenceAliases.push({ name: scopedName, kind: "forward" });
        ...
        for (const sourceName of resourceEntry.ctors ?? []) {
            ...
            if (typeof entry === "string") {
                utils_1.system.warn(`${sourceName} can not be a custom ctor, only objects containing name property can be accepted.`);
                continue;
            }
            entry.static = true;
    ...
    function detectMethods(sourceType, targetType, paramType, constParamType, blockedNames) {
        ...
        for (let [sourceName, transformEntry] of Object.entries(transformMap)) {
            ...
            if (hasPrefix)
                sourceName = sourceName.slice(prefix.length);
            if (blockedNames.has(sourceName))
                continue;
            ...
            if (sourceEntry) {
                ...
                if (placeAfter.has(lastKey))
                    placeAfter.get(lastKey).push(sourceName);
                else
                    placeAfter.set(lastKey, [sourceName]);
                blockedNames.add(sourceName);
            }
                ...
                if (blockedNames.has(methodName))
                    continue;
                if (transformEntry.after)
                    lastKey = transformEntry.after;
                if (placeAfter.has(lastKey))
                    placeAfter.get(lastKey).push(methodName);
                else
                    placeAfter.set(lastKey, [methodName]);
        ...
        for (const [sourceName, sourceEntry] of Object.entries(sourceEntries)) {
            if (blockedNames.has(sourceName) || foundEntries[sourceName])
                continue;
...
function makeSortedEntryArray(sourceEntries, file, context) {
    ...
    for (const [sourceName, transformEntry] of Object.entries(transformEntries)) {
        if (sourceEntries[sourceName]) {
            ...
            if (!transformEntry.name)
                transformEntry.name = sourceName;
            if (transformEntry.before) {
    ...
    if (includeBefore.__begin)
        addIncluded(includeBefore.__begin);
    if (includeAfter.__begin)
        addIncluded(includeAfter.__begin);
    for (const [key, sourceEntry] of Object.entries(sourceEntries)) {
        const sourceName = sourceEntry.name;
        if (processedSourceNames.has(sourceName))
            continue;
        ...
        if (key !== sourceName)
            addIncluded(includeBefore[key]);
        addIncluded(includeBefore[sourceName]);
        if (firstAppearance)
            addIncluded(includeBefore[targetName]);
        ...
        if (firstAppearance)
            addIncluded(includeAfter[targetName]);
        if (key !== sourceName)
            addIncluded(includeAfter[key]);
        addIncluded(includeAfter[sourceName]);
    }
    if (includeBefore.__end)
        addIncluded(includeBefore.__end);
    if (includeAfter.__end)
        addIncluded(includeAfter.__end);
    ...
    function addIncluded(transformEntries) {
        ...
        for (let i = 0; i < transformEntries.length; i++) {
            ...
            if (processedSourceNames.has(name)) {
                ...
                processedSourceNames.add(name);
            if (checkSubIncludes)
                addIncluded(includeBefore[name]);
            addTransform(transformEntry);
            if (checkSubIncludes)
                addIncluded(includeAfter[name]);
        }
    }
    /**
     *
     * @param {ApiEntryTransform} transformEntry
     */
    function addTransform(transformEntry) {
        ...
        (0, utils_1.combineObject)(currEntry, targetEntry);
    }
    /**
        ...
        function transformSubEntry(key, entry) {
            ...
            if (!sourceEntry) {
                if (typeof entry === "string") {
                    ...
                    (0, parse_1.insertEntry)(entries, /** @type {ApiEntry}*/ (entry), nameCandidate);
                }
                return;
            }
            const nameChange = makeRenameEntry(entry, nameCandidate, type);
            if (context.blacklist.has(key))
                addTransform(/** @type {ApiEntry}*/ (nameChange));
 ...
 */
function insertTransform(entries, transformEntry, defaultName = undefined) {
    if (defaultName)
...
function addHints(entry, hints) {
    if (entry.hints) {
        (0, utils_1.combineObject)(entry.hints, hints);
    }
    else {
        entry.hints = hints;
    }
...
function mirrorMethods(sourceEntries, transformEntries, transformSubEntries, paramType, constParamType, resultType) {
    for (const [sourceName, subEntry] of Object.entries(transformSubEntries)) {
        ...
        if (typeof subEntry === 'string') {
            ...
            targetEntry = (0, utils_1.deepClone)(subEntry);
            if (targetEntry.type === "")
                targetEntry.type = resultType;
            if (subEntry.name === "ctor")
                targetEntry.static = true;
        }
        ...
        if (transformEntry)
            (0, utils_1.combineObject)(targetEntry, transformEntry);
...
function scanFreeFunction(entries, uniqueType, pointerType) {
    ...
    for (const sourceName of Object.keys(entries)) {
        if ((0, utils_1.looksLikeFreeFunction)(sourceName)) {
...
function transformHierarchy(targetEntries, context) {
    for (const key of Object.keys(targetEntries)) {
        ...
        function insertCopyEntry(entry) {
            ...
            if (obj.hints)
                combineHints(entry, obj.hints);
            if (entry.static)
                addHints(entry, { static: true });
            delete entry.static;
...
function makeNaturalName(name, typeName) {
    ...
    if (!typeName.startsWith("F"))
        return name.replace(new RegExp(typeName), "");
...
function normalizeTypeName(typeName) {
    if (typeName.endsWith("Ref"))
        return typeName.slice(0, typeName.length - 3);
    return typeName;
}
/**
 *
...
function transformEntriesDocRefs(entries, context) {
    ...
    function transformEntryDoc(entry) {
        ...
        if (entry.overload)
            transformEntryDoc(entry.overload);
    }
}
/**
 * @param {string} doc
