...
class ProgListener implements CHeaderListener {
  ...
  enterProg(ctx: ProgContext) {
    const doc = ctx.doc();
    if (doc) this.api.parsedDoc = extractParsedDoc(this.api.name, doc.text);
  }

  enterDecl(ctx: DeclContext) {
    const doc = ctx.doc();
    if (doc && !this.api.parsedDoc) this.api.parsedDoc = extractParsedDoc(this.api.name, doc.text);
  }
  ...
  enterDirective(ctx: DirectiveContext) {
    ...
    const doc = extractParsedDoc(`${this.api.name}@${name}`, ctx.doc()?.text ?? (docIndex === -1 ? '' : directive.slice(docIndex).trim()));
    ...
    if (this.api.entries[name]?.parsedDoc) return;
  ...
  enterGlobalVar(ctx: GlobalVarContext) {
    ...
    const doc = extractParsedDoc(`${this.api.name}@${names[0]}`, ctx.doc()?.text ?? ctx.trailingDoc()?.text ?? '');
  ...
  enterFunctionDecl(ctx: FunctionDeclContext) {
    ...
    const doc = extractParsedDoc(`${this.api.name}@${name}`, ctx.doc()?.text ?? '');
...
function extractParsedDoc(name: string, text: string): ParsedDoc {
...
function extractEnumItems(prefix: string, ctx: EnumBodyContext): ApiEntries {
  ...
  for (const item of ctx.enumItem()) {
    ...
    entries[name] = {
      parsedDoc: extractParsedDoc(`${prefix}@${name}`, item.trailingDoc()?.text ?? item.doc()?.text ?? ''),
      name,
      kind: "var",
      type: "",
    };
...
function extractStructItems(prefix: string, ctx: StructBodyContext): ApiEntries {
  ...
  function addVar(item: StructVarContext) {
    ...
    const doc = extractParsedDoc(`${prefix}@${names[0]}`, item.trailingDoc()?.text ?? item.doc()?.text ?? '');
    for (const name of names) {
      entries[name] = {
        parsedDoc: doc,
        name,
  ...
  function addCallback(item: StructCallbackContext) {
    const type = extractType(item.type());
    const name = item.id().text;
    const doc = extractParsedDoc(`${prefix}@${name}`, item.trailingDoc()?.text ?? item.doc()?.text ?? '');
    entries[name] = {
      parsedDoc: doc,
      name,
      kind: 'var',
